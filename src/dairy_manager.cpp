...
// These functions are helpers for handling the CSV file format.

/**
 * @brief Safely prepares a string for writing to a CSV file by applying quoting and escaping.
 * @details See the detailed "Ruleset" documentation for a full explanation. In short, it
...
std::string escapeCsvField(const char *field)
{
    ...
    if (str_field.find(',') != std::string::npos ||
        ...
        for (char c : str_field) {
            if (c == '"') result += "\"\""; // An inner quote becomes two quotes.
            else result += c;
        }
...
bool parseCsvLineToEntry(const std::string &line, DiaryEntry& out_entry) \
{
    ...
    {
        char c = ss.peek(); // Look at the next character without consuming it.
        ...
        switch (field_index) {
            case 0: // ID
                try { out_entry.id = std::stoi(field); } 
                catch (...) { return false; /* If ID is not a number, the line is invalid. */ }
                break;
            case 1: // Date
...
bool DiaryManager::addEntry(const char *date, const char *title, const char *content) {
    if (entryExistsOnDate(date)) return false; //  No duplicate dates.
...
bool DiaryManager::updateEntry(int id, const char *date, const char *title, const char *content) {
    int index = findEntryIndex(id); // Find the entry in our array.
    if (index != -1) { // If found...
        // ...copy the new data over the old data.
        strncpy(entries[index].date, date, sizeof(entries[index].date) - 1);
        strncpy(entries[index].title, title, sizeof(entries[index].title) - 1);
        strncpy(entries[index].content, content, sizeof(entries[index].content) - 1);
        return true;
    }
    return false; // Return false if no entry with that ID exists.
}
...
bool DiaryManager::moveEntryUp(int id) {
    ...
    if (index > 0) { // Can't move up if it's the first one.
...
bool DiaryManager::moveEntryDown(int id) {
    int index = findEntryIndex(id);
    if (index != -1 && index < entry_count - 1) { // Can't move down if it's the last one.
...
// A private utility to find the array index (0, 1, 2...) for a given public ID (101, 102...).
...
void DiaryManager::resize_entry_array() {
    ...
    delete[] entries;
    //`entries` pointer to the new, larger array.
    entries = new_entries;
}
...
void DiaryManager::saveDataToFile() {
    ...
    file.close(); // Close the file to ensure data is flushed to disk.
}

// The "Two-Pass" loading method.
void DiaryManager::loadDataFromFile() {
    std::ifstream file(filename);
    if (!file.is_open()) return; // File doesn't exist yet, nothing to load.
    ...
    if (entry_line_count == 0) {
        file.close();
        return; // No entries in the file.
    }
    
    // 
    delete[] entries; // Free any existing memory.
    entry_count = entry_line_count;
    entry_capacity = entry_line_count;
    entries = new DiaryEntry[entry_capacity];

    // --- PASS 2: Rewind the file and read the data into our new array.
    ...
    while (std::getline(file, line)) {
        ...
        if (parseCsvLineToEntry(line, entries[current_index])) {
            // Keep track of the highest ID to set our ID generator correctly.
    ...
    // The final count is how many lines we *successfully* parsed.
    entry_count = current_index;
    // Ensure new entries don't have conflicting IDs.
    next_id = max_id + 1;
